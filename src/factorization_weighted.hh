...
auto
make_factorization_weighted_eta(const Eigen::MatrixBase<Derived> &data,
                                UParam &u,
                                VParam &v)
{
...
template <typename Derived, typename UParam, typename VParam>
auto
make_factorization_weighted_eta_ptr(const Eigen::MatrixBase<Derived> &data,
                                    UParam &u,
                                    VParam &v)
{
    using Scalar = typename Derived::Scalar;
...
struct factorization_weighted_t {
    ...
    template <typename RNG>
    inline const auto &sample(RNG &rng)
    {
    ...
    {
        grad_v_mean = Eta.get_grad_type1().transpose() *
        ...
        grad_v_var = Eta.get_grad_type2().transpose() * uw_sq +
            Eta.get_grad_type2().transpose() *
                (var_param(U).cwiseProduct(weight_nk).cwiseProduct(weight_nk));
    }
