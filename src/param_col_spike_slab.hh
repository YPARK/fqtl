...
template <typename Scalar, typename Index, typename Opt>
auto make_dense_col_spike_slab_ptr(const Index n1, const Index n2, const Opt& opt) {
  using Mat = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>;
  using Param = param_col_spike_slab_t<Mat, tag_param_dense>;
...
auto make_sparse_col_spike_slab(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
...
void impl_update_param_sgd(Parameter& P, const Scalar rate, const tag_param_col_spike_slab) {
...
template <typename Parameter, typename Scalar>
void impl_update_hyperparam_sgd(Parameter& P, const Scalar rate, const tag_param_col_spike_slab) {
...
template <typename Parameter, typename Scalar>
void impl_perturb_param(Parameter& P, const Scalar sd, const tag_param_col_spike_slab) {
  std::mt19937 rng;
...
auto impl_log_odds_param(Parameter& P, const tag_param_col_spike_slab) {
  return P.alpha_aux_col.unaryExpr([&P](const auto& x) { return P.pi_aux + x; });
}
...
void impl_eval_param_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs, const tag_param_col_spike_slab) {
  ////////////////////////////////
...
template <typename Parameter, typename M1, typename M2, typename M3>
void impl_eval_hyperparam_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs,
...
template <typename Parameter>
void impl_write_param(Parameter& P, const std::string hdr, const std::string gz, const tag_param_col_spike_slab) {
  write_data_file((hdr + ".theta" + gz), P.theta);
  write_data_file((hdr + ".theta_var" + gz), P.theta_var);
  typename Parameter::data_t temp = P.alpha_aux_col.unaryExpr([&P](const auto& x) { return P.pi_aux + x; });
