...
struct factored_weighted_regression_t {
  ...
  inline void resolve() {
    ...
    update_mean(Eta, (X * ThetaL.theta).cwiseProduct(weight_nk) *
    ...
    //           (mean_left_tot^2) * V[R'] +
    //           var_left_tot * (E[R']^2)
    update_var(Eta, var_nk * ThetaR.theta_var.transpose() + /* var x var */
  ...
  inline void eval_sgd() {
    ...
    trans_times_add(Eta.get_grad_type1(), temp_nk.cwiseProduct(weight_nk),
                    G1R); /* (n x m)' (n x k) = (m x k) */
...
struct get_factored_weighted_regression_type<
  ...
  using type = factored_weighted_regression_t<DenseReprMat<Scalar>, ParamLeft,
                                              ParamRight>;
};
...
auto make_factored_weighted_regression_eta(
    ...
    const Eigen::MatrixBase<yDerived> &yy, ParamLeft &thetaL,
    ParamRight &thetaR) {
  using Scalar = typename yDerived::Scalar;
  using Reg = factored_weighted_regression_t<DenseReprMat<Scalar>, ParamLeft,
                                             ParamRight>;
