...
struct beta_model_t {
  ...
  explicit beta_model_t(const T& Y, const beta_hyper_t& beta_tol,
 ...
 private:
  ...
  template <typename Derived>
  static void preprocess(const Dense<Derived>& y_raw, Dense<Derived>& logit_y,
                         Dense<Derived>& log1pMinus_y) {
    _preprocess(y_raw.derived(), logit_y.derived(), log1pMinus_y.derived());
  }

  template <typename Derived>
  static void preprocess(const Sparse<Derived>& y_raw, Sparse<Derived>& logit_y,
  ...
  struct rbeta_op_t {
    explicit rbeta_op_t(const Scalar tol) : TOL(tol), rng(std::time(0)) {}
    ...
    Scalar operator()(const Scalar& mu_val, const Scalar& phi_val) {
      ...
      const Scalar b = rgammaB(
          rng, gamma_param_type(TOL + (one_val - mu_val) * phi_val, one_val));
      return a / (a + b);
    }
    const Scalar TOL;
