...
struct gaussian_voom_model_t {
  ...
  explicit gaussian_voom_model_t(const T& yy, const Vmin_t& Vmin, const Vmax_t& Vmax)
      : n(yy.rows()),
        m(yy.cols()),
        Y_safe(n, m),
        llik_mat(n, m),
 ...
 private:
  ...
  const T& _eval(const M1& eta_mean, const M2& eta_var) {
    ...
    llik_mat = -half_val * var_mat.unaryExpr(log_op) - half_val * Y_safe.binaryExpr(mean_mat, mean_op).cwiseQuotient(var_mat);
