...
auto make_sparse_slab(const Eigen::SparseMatrixBase<Derived>& A, const Opt& opt) {
...
void impl_perturb_param(Parameter& P, const Scalar sd, const tag_param_slab) {
  std::mt19937 rng;
...
void impl_eval_param_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs, const tag_param_slab) {
  ////////////////////////////////
  // gradient w.r.t. beta
...
template <typename Parameter, typename M1, typename M2, typename M3>
void impl_eval_hyperparam_sgd(Parameter& P, const M1& G1, const M2& G2, const M3& Nobs, const tag_param_slab) {
...
template <typename Parameter, typename Scalar>
void impl_update_param_sgd(Parameter& P, const Scalar rate, const tag_param_slab) {
...
template <typename Parameter, typename Scalar>
void impl_update_hyperparam_sgd(Parameter& P, const Scalar rate, const tag_param_slab) {
  P.tau_aux += update_adam(P.adam_tau_aux, P.grad_tau_aux) * rate;
  resolve_hyperparam(P);
}

template <typename Parameter>
void impl_write_param(Parameter& P, const std::string hdr, const std::string gz, const tag_param_slab) {
