fqtl.mf <- function(y, x.mean = NULL, x.var = NULL, c.mean = NULL, y.loc = NULL, y.loc2 = NULL, x.mean.loc = NULL,
                    cis.dist = 5e5, options = list(model='gaussian', vbiter=1000, tol=1e-8, gammax=100,
...
fqtl.regress <- function(y, x.mean, factored = FALSE, c.mean = NULL, c.mean.t = NULL, x.var = NULL,
    ...
    if(is.null(c.mean.t)){ c.mean.t <- matrix(0, nrow=1, ncol=m) }
    if(is.null(x.var)){ x.var <- matrix(1, nrow=n, ncol=1) }

    stopifnot(dim(x.mean)[1] == n)
    stopifnot(dim(x.var)[1] == n)
    stopifnot(dim(c.mean.t)[2] == m)

    if(factored){

        if(is.null(y.loc) || is.null(c.mean.loc)){
            return(.Call('fqtl_rcpp_train_freg', PACKAGE = 'fqtl', y, x.mean, c.mean, c.mean.t, x.var, options))
        }
        ...
        return(.Call('fqtl_rcpp_train_freg_cis', PACKAGE = 'fqtl', y, x.mean, c.mean, c.mean.adj, c.mean.t, x.var, options))

    } else {

        if(is.null(y.loc) || is.null(x.mean.loc)){
            return(.Call('fqtl_rcpp_train_reg', PACKAGE = 'fqtl', y, x.mean, c.mean, c.mean.t, x.var, options))
        }
        ...
        if(is.null(c.mean.loc)){
            return(.Call('fqtl_rcpp_train_reg_cis', PACKAGE = 'fqtl', y, x.mean, x.mean.adj, c.mean, c.mean.t, x.var, options))
        }
        ...
        return(.Call('fqtl_rcpp_train_reg_cis_cis', PACKAGE = 'fqtl',
                     y, x.mean, x.mean.adj, c.mean, c.mean.adj, c.mean.t, x.var, options))
    }
}
