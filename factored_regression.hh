...
struct factored_regression_t {
  ...
  explicit factored_regression_t (const ReprMatrix &xx, const ReprMatrix &yy, ParamLeft &thetaL, ParamRight &thetaR)
      : n (xx.rows ()),
        p (xx.cols ()),
        m (yy.cols ()),
        k (thetaL.cols ()),
        ...
        Eta (make_gaus_repr (yy))
  {
    ...
    remove_missing (xx, X);
    remove_missing (xx.unaryExpr ([] (const auto &x)
                                  { return x * x; }), Xsq);
  ...
  template<typename RNORM>
  const ReprMatrix &sample (const RNORM &rnorm)
  {
    return sample_repr (Eta, rnorm);
  }

  const ReprMatrix &repr_mean () const
  { return Eta.get_mean (); }

  const ReprMatrix &repr_var () const
  { return Eta.get_var (); }

  void add_sgd (const ReprMatrix &llik)
  { update_gradient (Eta, llik); }

  // mean = X * E[L] * E[R]'
  // var = X^2 * (Var[L] * Var[R]' + E[L]^2 * Var[R]' + Var[L] * E[R']^2)
  void resolve ()
  {
    thetaRsq = mean_param (ThetaR).cwiseProduct (mean_param (ThetaR)); /* (m x k)  */
    thetaLsq = mean_param (ThetaL).cwiseProduct (mean_param (ThetaL)); /* (n x k)  */

    update_mean (Eta, X * mean_param (ThetaL) * mean_param (ThetaR).transpose ()); /* mean x mean */
    update_var (Eta, Xsq * (var_param (ThetaL) * var_param (ThetaR).transpose () + /* var x var */
  ...
  void eval_sgd ()
  {
    ...
    times_set (Eta.get_grad_type2 (), var_param (ThetaR), temp_nk); /* (n x m) (m x k) = (n x k)  */
    trans_times_set (Xsq, temp_nk, G1L);                          /* (n x p)' (n x k) = (p x k) */
    G1L = 2.0 * G1L.cwiseProduct (mean_param (ThetaL));            /* (p x k)                    */

    times_set (Eta.get_grad_type1 (), mean_param (ThetaR), temp_nk); /* (n x m) (m x k) = (n x k) */
    trans_times_add (X, temp_nk, G1L);

    // (2) update of G2L
    times_set (Eta.get_grad_type2 (), var_param (ThetaR), temp_nk); /* (n x m) (m x k) = (n x k)  */
    times_add (Eta.get_grad_type2 (), thetaRsq, temp_nk);          /* (n x m) (m x k) = (n x k)  */
    ...
    trans_times_set (Eta.get_grad_type2 (), temp_nk, G1R); /* (n x m)' (n x k) = (m x k) */
    G1R = 2.0 * G1R.cwiseProduct (mean_param (ThetaR));    /* (m x k) */

    times_set (X, mean_param (ThetaL), temp_nk);           /* (n x p) (p x k) = (n x k) */
    trans_times_add (Eta.get_grad_type1 (), temp_nk, G1R); /* (n x m)' (n x k) = (m x k) */

    // (4) update of G2R
    times_set (Xsq, var_param (ThetaL), temp_nk);          /* (n x p) (p x k) = (n x k) */
    times_add (Xsq, thetaLsq, temp_nk);                   /* (n x p) (p x k) = (n x k) */
    trans_times_set (Eta.get_grad_type2 (), temp_nk, G2R); /* (n x m)' (n x k) = (m x k) */
...
template<typename ParamLeft, typename ParamRight, typename Scalar, typename Matrix>
...
struct get_factored_regression_type<ParamLeft, ParamRight, Scalar,
  ...
  using type = factored_regression_t<DenseReprMat<Scalar>, ParamLeft, ParamRight>;
};

template<typename ParamLeft, typename ParamRight, typename Scalar>
struct get_factored_regression_type<ParamLeft, ParamRight, Scalar, Eigen::SparseMatrix<Scalar> > {
  using type = factored_regression_t<SparseReprMat<Scalar>, ParamLeft, ParamRight>;
};

template<typename xDerived, typename yDerived, typename ParamLeft, typename ParamRight>
auto make_factored_regression_eta (const Eigen::MatrixBase<xDerived> &xx, const Eigen::MatrixBase<yDerived> &yy,
                                   ParamLeft &thetaL, ParamRight &thetaR)
{
  using Scalar = typename yDerived::Scalar;
  using Reg = factored_regression_t<DenseReprMat<Scalar>, ParamLeft, ParamRight>;
  return Reg (xx.derived (), yy.derived (), thetaL, thetaR);
}

template<typename xDerived, typename yDerived, typename ParamLeft, typename ParamRight>
auto make_factored_regression_eta (const Eigen::SparseMatrixBase<xDerived> &xx,
                                   const Eigen::SparseMatrixBase<yDerived> &yy, ParamLeft &thetaL, ParamRight &thetaR)
{
  using Scalar = typename yDerived::Scalar;
  using Reg = factored_regression_t<SparseReprMat<Scalar>, ParamLeft, ParamRight>;
  return Reg (xx.derived (), yy.derived (), thetaL, thetaR);
}

template<typename xDerived, typename yDerived, typename ParamLeft, typename ParamRight>
auto make_factored_regression_eta_ptr (const Eigen::MatrixBase<xDerived> &xx, const Eigen::MatrixBase<yDerived> &yy,
                                       ParamLeft &thetaL, ParamRight &thetaR)
{
  using Scalar = typename yDerived::Scalar;
  using Reg = factored_regression_t<DenseReprMat<Scalar>, ParamLeft, ParamRight>;
...
template<typename xDerived, typename yDerived, typename ParamLeft, typename ParamRight>
auto make_factored_regression_eta_ptr (const Eigen::SparseMatrixBase<xDerived> &xx,
